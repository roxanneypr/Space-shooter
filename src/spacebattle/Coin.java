...
class Coin extends Powerups {
	public final static Image COIN_IMAGE = new Image("images/goldcoin.png");
	...
	@Override
	void checkCollision(Spaceship spaceship){		//checks if the spaceship has collided with the coin
		if(this.collidesWith(spaceship)){
			System.out.println(spaceship.getName() + " has collected a coin!");
			this.vanish();
			spaceship.setCoin();
			spaceship.addSpeed();
		}
